...
def test_performance_gnn_inter(model, dataloader, test_interpro, gog_matrix, test_pid_list, test_go, idx_goid, goid_idx, ont, device, save=False, save_file=None, evaluate=True, with_relations=True):
    model.eval()
    ...
    for batch_idx, (x_test, sample_idx, y_test) in enumerate(dataloader):
        ...
        feats1 = x_test.ndata['x']
        feats2 = x_test.ndata['y'][:, 0:8]
        feats3 = x_test.ndata['y'][:, 8: ]

        inter_features = torch.from_numpy(test_interpro[sample_idx]).to(device).float()
        
        y_pred = model(x_test, feats1, feats2, feats3, inter_features, gog_tensor)
