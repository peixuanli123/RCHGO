...
pssm_feature_dir = "/data/yihengzhu/MKFGO_2.0/resource/benchmark/pssm/logpssm/"
pss_feature_dir = "/data/yihengzhu/MKFGO_2.0/resource/benchmark/psd_array/"
...
def read_pssm_feature(feature_file):   # read features from transformer
...
def read_pss_feature(feature_file):   # read features from transformer
...
def get_whole_pdb_graph(pdb_points, pid_list, sequence_dict, thresholds, ont, tag):
    ...
    for pid in tqdm(pid_list):
        ...
        pssm_feature = read_pssm_feature(pssm_feature_dir + "/" + pid + ".pssm")
        pss_feature = read_pss_feature(pss_feature_dir + "/" + pid + ".npy")

        assert len(sequence1) == pssm_feature.shape[0]
        assert len(sequence1) == pss_feature.shape[0]
        ...
        # graph node feature
        graph.ndata['x'] = torch.zeros(graph.num_nodes(), 20)
        graph.ndata['y'] = torch.zeros(graph.num_nodes(), 13)
        ...
        for node_id in range(len(points)):
            ...
            graph.ndata['x'][node_id] = torch.from_numpy(pssm_feature[node_id])
            graph.ndata['y'][node_id] = torch.from_numpy(pss_feature[node_id])
