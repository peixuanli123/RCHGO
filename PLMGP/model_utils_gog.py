...
def test_performance_gnn_inter(model, dataloader, gog_matrix, test_pid_list, test_go, idx_goid, goid_idx, ont, device, save=False, save_file=None, evaluate=True, with_relations=True):
    model.eval()
    ...
    for batch_idx, (x_test, sample_idx, y_test) in enumerate(dataloader):
        ...
        y_pred = model(x_test, feats, gog_tensor.to(device))
