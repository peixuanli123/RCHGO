...
from DPFunc.data_utils import get_pdb_data, get_mlb
from DPFunc.models_term_specific import combine_inter_model
from DPFunc.objective import AverageMeter
from DPFunc.model_utils_gog import test_performance_gnn_inter, merge_result, FocalLoss
...
def main(data_cnf, gpu_number, epoch_number, round_index):
    ...
    f = open(data_cnf["term_list"], "w")
    for term in mlb.classes_:
        f.write(term + "\n")
    f.close()
    ...
    model = combine_inter_model(graph_size=1024,
                                graph_hid=1024,
                                label_num=labels_num, head=2).to(device)
    logger.info(model)
    ...
    for e in range(1, epoch_number + 1):
        ...
        for batched_graph, sample_idx, labels in tqdm(train_dataloader, leave=False):
            ...
            logits = model(batched_graph, feats, gog_tensor)
        ...
        model_eval = combine_inter_model(graph_size=1024, graph_hid=1024, label_num=labels_num, head=2).to(eval_device)
        model_eval.load_state_dict(model.state_dict())
        model_eval.eval()
        
        valid_plus_fmax, valid_plus_aupr, valid_plus_t, valid_df, valid_loss_avg = test_performance_gnn_inter(model_eval, valid_dataloader, gog_matrix, valid_pid_list, valid_y, idx_goid, goid_idx, ont, eval_device, save=True, save_file=evaluate_result_file, evaluate=True)
        ...
        test_plus_fmax, test_plus_aupr, test_plus_t, test_df, test_loss_avg = test_performance_gnn_inter(model_eval, test_dataloader, gog_matrix, test_pid_list, test_y, idx_goid, goid_idx, ont, eval_device, save=True, save_file=test_result_file, evaluate=True)
