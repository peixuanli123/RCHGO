...
def read_fasta( fasta_path ):
    ...
    with open( fasta_path, 'r' ) as fasta_f:
        ...
        for uniprot_id in sequences:
            current_sequence = sequences[ uniprot_id ]
            if(len(current_sequence)>1024):
                current_sequence = current_sequence[0:1024]
            sequences[uniprot_id] = current_sequence
...
def get_embeddings( seq_path, 
    ...
    for sequence_id, embedding in emb_dict.items():
        ...
        feature_file = str(emb_path) + "/" + str(sequence_id) + ".pt"
